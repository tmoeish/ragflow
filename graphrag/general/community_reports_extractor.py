...
from graphrag.utils import dict_has_keys_with_types, perform_variable_replacements
    ...
    def __call__(self, graph: nx.Graph, callback: Callable | None = None):
        ...
        for level, comm in communities.items():
            ...
            for cm_id, ents in comm.items():
                ...
                ents = ents["nodes"]
                ent_df = pd.DataFrame(
                    self._get_entity_(ents)
                ).dropna()  # [{"entity": n, **graph.nodes[n]} for n in ents])
                ...
                rela_df = pd.DataFrame(
                    self._get_relation_(
                        list(ent_df["entity"]), list(ent_df["entity"]), 10000
                    )
                )
                if rela_df.empty:
                    continue
                ...
                text = perform_variable_replacements(
                    self._extraction_prompt, variables=prompt_variables
                )
                ...
                if callback:
                    callback(
                        msg=f"Communities: {over}/{total}, elapsed: {timer() - st}s, used tokens: {token_count}"
                    )
