...
class Dealer:
    def __init__(
        ...
        for en in ents:
            self.graph.add_node(
                en["entity_name"], entity_type=en["entity_type"]
...
class WithResolution(Dealer):
    def __init__(
        ...
        with RedisDistributedLock(kb_id, 60 * 60):
            ...
            if not self.graph:
                logging.error(
                    f"Faild to fetch the graph. tenant_id:{kb_id}, kb_id:{kb_id}"
                )
                if callback:
            ...
            logging.info(
                "Graph resolution is done. Remove {} nodes.".format(
                    len(reso.removed_entities)
                )
            )
            if callback:
                callback(
                    msg="Graph resolution is done. Remove {} nodes.".format(
                        len(reso.removed_entities)
                    )
                )
            update_nodes_pagerank_nhop_neighbour(tenant_id, kb_id, self.graph, 2)
...
class WithCommunity(Dealer):
    def __init__(
        ...
        if callback:
            callback(
                msg="Graph community extraction is done. Indexing {} reports.".format(
                    len(cr.structured_output)
                )
            )
