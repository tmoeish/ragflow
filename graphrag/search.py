...
class KGSearch(Dealer):
    ...
    def query_rewrite(self, llm, question, idxnms, kb_ids):
        ...
        hint_prompt = PROMPTS["minirag_query2kwd"].format(
            query=question, TYPE_POOL=json.dumps(ty2ents, ensure_ascii=False, indent=2)
        )
        result = self._chat(
            llm,
            hint_prompt,
    ...
    def _relation_info_from_(self, es_res, sim_thr=0.3):
        res = {}
        es_res = self.dataStore.getFields(
            es_res,
            [
                "content_with_weight",
                "_score",
    ...
    def retrieval(
        ...
        ty_kwds = []
        try:
            ty_kwds, ents = self.query_rewrite(
                llm, qst, [index_name(tid) for tid in tenant_ids], kb_ids
            )
        ...
        logging.info(
            "Retrieved entities from types({}): {}".format(
                ty_kwds, list(ents_from_types.keys())
            )
        )
        ...
        ents_from_query = sorted(
            ents_from_query.items(),
            key=lambda x: x[1]["sim"] * x[1]["pagerank"],
            reverse=True,
        )[:ent_topn]
        rels_from_txt = sorted(
            rels_from_txt.items(),
            key=lambda x: x[1]["sim"] * x[1]["pagerank"],
            reverse=True,
        )[:rel_topn]
            ...
            ents.append(
                {
                    ...
                    "Description": json.loads(ent["description"]).get(
                        "description", ""
                    ),
                }
            )
        ...
        return {
            "chunk_id": get_uuid(),
            "content_ltks": "",
            "content_with_weight": ents
            + relas
            + self._community_retrival_(
                [n for n, _ in ents_from_query],
                filters,
                kb_ids,
                idxnms,
                comm_topn,
    ...
    def _community_retrival_(
        self, entities, condition, kb_ids, idxnms, topn, max_token
    ):
        ## Community retrieval
        ...
        comm_res = self.dataStore.search(
            fields, [], fltr, [], OrderByExpr(), 0, topn, idxnms, kb_ids
        )
...
if __name__ == "__main__":
    ...
    print(
        kg.retrieval(
            {"question": args.question, "kb_ids": [kb_id]},
