...
class GraphExtractor(Extractor):
    ...
    def __init__(
        ...
        self._left_token_count = max(
            llm_invoker.max_length * 0.6, self._left_token_count
        )
    ...
    def _process_single_content(self, chunk_key_dp: tuple[str, str]):
        ...
        try:
            ...
            final_result = self._chat(
                hint_prompt, [{"role": "user", "content": "Output:"}], gen_conf
            )
            token_count += num_tokens_from_string(hint_prompt + final_result)
            history = pack_user_ass_to_openai_messages(
                "Output:", final_result, self._continue_prompt
            )
            ...
            records = split_string_by_multi_markers(
                ...
                [
                    ...
                    self._context_base["completion_delimiter"],
                ],
            )
            rcds = []
            ...
            maybe_nodes, maybe_edges = self._entities_and_relations(
                chunk_key, records, self._context_base["tuple_delimiter"]
            )
