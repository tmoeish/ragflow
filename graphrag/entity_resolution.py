...
class EntityResolution(Extractor):
    ...
    def __call__(
        self, graph: nx.Graph, prompt_variables: dict[str, Any] | None = None
        ...
        prompt_variables = {
            **prompt_variables,
            ...
            self._entity_index_dilimiter_key: prompt_variables.get(
                self._entity_index_dilimiter_key
            )
            or DEFAULT_ENTITY_INDEX_DELIMITER,
            self._resolution_result_delimiter_key: prompt_variables.get(
        ...
        nodes = graph.nodes
        entity_types = list(
            set(graph.nodes[node].get("entity_type", "-") for node in nodes)
        )
            ...
            if candidate_resolution_i[1]:
                try:
                    ...
                    sent = (
                        "question above"
                        if len(pair_txt) == 1
                        else f"above {len(pair_txt)} questions"
                    )
                    pair_txt.append(
                    ...
                    variables = {**prompt_variables, self._input_text_key: pair_prompt}
                    text = perform_variable_replacements(
                        self._resolution_prompt, variables=variables
                    )

                    response = self._chat(
                        text, [{"role": "user", "content": "Output:"}], gen_conf
                    )
                    ...
                    for result_i in result:
                        resolution_result.add(
                            candidate_resolution_i[1][result_i[0] - 1]
                        )
                except Exception:
        ...
        return EntityResolutionResult(graph=graph, removed_entities=removed_entities)
