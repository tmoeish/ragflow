...
class QWenChat(Base):
    def __init__(self, key, model_name=Generation.Models.qwen_turbo, **kwargs):
        ...
        if model_name.lower().find("deepseek") >= 0:
            super().__init__(
                key, model_name, "https://dashscope.aliyuncs.com/compatible-mode/v1"
            )
    ...
    def chat(self, system, history, gen_conf):
        ...
        stream_flag = (
            str(os.environ.get("QWEN_CHAT_BY_STREAM", "true")).lower() == "true"
        )
        if not stream_flag:
            ...
            result_list = list(g)
            error_msg_list = [
                item for item in result_list if str(item).find("**ERROR**") >= 0
            ]
...
class OllamaChat(Base):
    ...
    def chat(self, system, history, gen_conf):
        ...
        try:
            ...
            return ans, response.get("eval_count", 0) + response.get(
                "prompt_eval_count", 0
            )
        except Exception as e:
            return "**ERROR**: " + str(e), 0
    ...
    def chat(self, system, history, gen_conf):
        ...
        try:
            response = self.client.do(
                model=self.model_name, messages=history, system=self.system, **gen_conf
            ).body
            ans = response["result"]
            return ans, self.total_token_count(response)
