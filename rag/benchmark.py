...
class Benchmark:
    def __init__(self, kb_id):
        ...
        self.embd_mdl = LLMBundle(
            self.kb.tenant_id,
            LLMType.EMBEDDING,
            llm_name=self.kb.embd_id,
    ...
    def ms_marco_index(self, file_path, index_name):
        ...
        for fn in filelist:
            ...
            for i in tqdm(range(len(data)), colour="green", desc="Tokenizing:" + fn):
                ...
                for rel, text in zip(
                    data.iloc[i]["passages"]["is_selected"],
    ...
    def trivia_qa_index(self, file_path, index_name):
        ...
        for fn in filelist:
            ...
            for i in tqdm(range(len(data)), colour="green", desc="Indexing:" + fn):
                ...
                for rel, text in zip(
                    data.iloc[i]["search_results"]["rank"],
    ...
    def miracl_index(self, file_path, corpus_path, index_name):
        ...
        for topics_file in os.listdir(os.path.join(file_path, "topics")):
            ...
            tmp_data = pd.read_csv(
                os.path.join(file_path, "topics", topics_file),
                sep="\t",
                names=["qid", "query"],
            )
            for index, i in tmp_data.iterrows():
        ...
        for qrels_file in os.listdir(os.path.join(file_path, "qrels")):
            ...
            tmp_data = pd.read_csv(
                os.path.join(file_path, "qrels", qrels_file),
                sep="\t",
                names=["qid", "Q0", "docid", "relevance"],
            )
            for i in tqdm(
                range(len(tmp_data)), colour="green", desc="Indexing:" + qrels_file
            ):
                if docs_count >= max_docs:
                    break
    ...
    def save_results(self, qrels, run, texts, dataset, file_path):
        ...
        for run_i in tqdm(
            range(len(run_keys)), desc="Calculating ndcg@10 for single query"
        ):
            key = run_keys[run_i]
            keep_result.append(
                {
        ...
        with open(
            os.path.join(file_path, dataset + "result.md"), "w", encoding="utf-8"
        ) as f:
        ...
        json.dump(
            qrels,
            open(
                os.path.join(file_path, dataset + ".qrels.json"), "w+", encoding="utf-8"
            ),
            indent=2,
        )
        json.dump(
            run,
            open(
                os.path.join(file_path, dataset + ".run.json"), "w+", encoding="utf-8"
            ),
            indent=2,
        )
    ...
    def __call__(self, dataset, file_path, miracl_corpus=""):
        if dataset == "ms_marco_v1.1":
            ...
            print(
                dataset,
                evaluate(Qrels(qrels), Run(run), ["ndcg@10", "map@5", "mrr@10"]),
            )
        ...
        if dataset == "miracl":
            for lang in [
                ...
                if not os.path.isdir(os.path.join(file_path, "miracl-v1.0-" + lang)):
                    print(
                        "Directory: "
                        + os.path.join(file_path, "miracl-v1.0-" + lang)
                        + " not found!"
                    )
                    continue
                if not os.path.isdir(
                    os.path.join(file_path, "miracl-v1.0-" + lang, "qrels")
                ):
                    print(
                        "Directory: "
                        + os.path.join(file_path, "miracl-v1.0-" + lang, "qrels")
                    ...
                    continue
                if not os.path.isdir(
                    os.path.join(file_path, "miracl-v1.0-" + lang, "topics")
                ):
                    print(
                        "Directory: "
                        + os.path.join(file_path, "miracl-v1.0-" + lang, "topics")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="benchmark.py <max_docs> <kb_id> <dataset> <dataset_path> [<miracl_corpus_path>])",
        description="RAGFlow Benchmark",
    )
    parser.add_argument(
        "max_docs", metavar="max_docs", type=int, help="max docs to evaluate"
    )
    ...
    parser.add_argument(
        "miracl_corpus_path",
        metavar="miracl_corpus_path",
        nargs="?",
        default="",
        help="miracl corpus path. Only needed when dataset is miracl",
    )
