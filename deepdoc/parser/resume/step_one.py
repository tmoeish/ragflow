...
def refactor(df):
            ...
            if cc:
                ...
                df[c] = df["obj"].map(
                    lambda x: (
                        ...
                        and (isinstance(x.get(c), type({})) or not x.get(c))
                        else str(x).replace("None", "")
                    )
                )
    ...
    df["industry_names"] = df["industry_ids"].map(
        lambda x: " ".join(
            [" ".join(industries.get_names(i)) for i in str(x).split(",")]
        )
    )
    ...
    df["gender"] = df["gender"].map(
        lambda x: "男" if x == "M" else ("女" if x == "F" else "")
    )
    for c in [
        "is_fertility",
        "is_oversea",
