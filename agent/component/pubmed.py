...
class PubMed(ComponentBase, ABC):
    ...
    def _run(self, history, **kwargs):
        ...
        try:
            ...
            pubmedids = Entrez.read(
                Entrez.esearch(db="pubmed", retmax=self._param.top_n, term=ans)
            )["IdList"]
            pubmedcnt = ET.fromstring(
                re.sub(
                    r"<(/?)b>|<(/?)i>",
                    "",
                    Entrez.efetch(db="pubmed", id=",".join(pubmedids), retmode="xml")
                    .read()
                    .decode("utf-8"),
                )
            )
