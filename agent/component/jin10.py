...
class Jin10Param(ComponentParamBase):
    ...
    def check(self):
        self.check_valid_value(
            self.type, "Type", ["flash", "calendar", "symbols", "news"]
        )
        self.check_valid_value(self.flash_type, "Flash Type", ["1", "2", "3", "4", "5"])
        self.check_valid_value(
            self.calendar_type, "Calendar Type", ["cj", "qh", "hk", "us"]
        )
        self.check_valid_value(
            self.calendar_datatype, "Calendar DataType", ["data", "event", "holiday"]
        )
        self.check_valid_value(
            self.symbols_type, "Symbols Type", ["GOODS", "FOREX", "FUTURE", "CRYPTO"]
        )
        self.check_valid_value(
            self.symbols_datatype, "Symbols DataType", ["symbols", "quotes"]
        )
        ...
        try:
            if self._param.type == "flash":
                ...
                response = requests.get(
                    url="https://open-data-api.jin10.com/data-api/flash?category="
                    + self._param.flash_type,
                    headers=headers,
            ...
            if self._param.type == "calendar":
                ...
                response = requests.get(
                    url="https://open-data-api.jin10.com/data-api/calendar/"
                    + self._param.calendar_datatype
                    + "?category="
                ...
                response = response.json()
                jin10_res.append(
                    {"content": pd.DataFrame(response["data"]).to_markdown()}
                )
            if self._param.type == "symbols":
                ...
                if self._param.symbols_datatype == "quotes":
                    params["codes"] = "BTCUSD"
                response = requests.get(
                    url="https://open-data-api.jin10.com/data-api/"
                    + self._param.symbols_datatype
                    + "?type="
