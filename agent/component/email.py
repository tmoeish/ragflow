...
class Email(ComponentBase, ABC):
    ...
    def _run(self, history, **kwargs):
        ...
        try:
            ...
            msg["From"] = formataddr(
                (str(Header(self._param.sender_name, "utf-8")), self._param.email)
            )
            msg["To"] = email_data["to_email"]
            if "cc_email" in email_data and email_data["cc_email"]:
                msg["Cc"] = email_data["cc_email"]
            msg["Subject"] = Header(
                email_data.get("subject", "No Subject"), "utf-8"
            ).encode()
            ...
            logging.info(
                f"Connecting to SMTP server {self._param.smtp_server}:{self._param.smtp_port}"
            )

            context = smtplib.ssl.create_default_context()
            with smtplib.SMTP_SSL(
                self._param.smtp_server, self._param.smtp_port, context=context
            ) as server:
                # Login
                ...
                try:
                    ...
                    logging.warning(
                        f"Non-fatal error during connection close: {str(e)}"
                    )
