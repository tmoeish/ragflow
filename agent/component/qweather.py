...
class QWeatherParam(ComponentParamBase):
    ...
    def check(self):
        ...
        self.check_valid_value(
            self.user_type, "Free subscription or paid subscription", ["free", "paid"]
        )
        self.check_valid_value(
            self.lang,
            "Use language",
        ...
        self.check_valid_value(
            self.time_period, "Time period", ["now", "3d", "7d", "10d", "15d", "30d"]
        )
    ...
    def _run(self, history, **kwargs):
        ...
        try:
            response = requests.get(
                ...
                + ans
                + "&key="
                + self._param.web_apikey
            ).json()
            if response["code"] == "200":
                location_id = response["location"][0]["id"]
            else:
                return QWeather.be_output(
                    "**Error**" + self._param.error_code[response["code"]]
                )

            base_url = (
                "https://api.qweather.com/v7/"
                if self._param.user_type == "paid"
