...
class GoogleScholarParam(ComponentParamBase):
    ...
    def check(self):
        ...
        self.check_valid_value(
            self.sort_by, "GoogleScholar Sort_by", ["date", "relevance"]
        )
        self.check_boolean(
            self.patents, "Whether or not to include patents, defaults to True"
        )
...
class GoogleScholar(ComponentBase, ABC):
    ...
    def _run(self, history, **kwargs):
        ...
        scholar_client = scholarly.search_pubs(
            ans,
            patents=self._param.patents,
            year_low=self._param.year_low,
            year_high=self._param.year_high,
                ...
                scholar_res.append(
                    {
                        ...
                        + ",".join(pub["bib"]["author"])
                        + "\n Abstract: "
                        + pub["bib"].get("abstract", "no abstract")
                    }
                )
