...
class DocumentService(CommonService):
    ...
    def increment_chunk_num(cls, doc_id, kb_id, token_num, chunk_num, duation):
        num = (
            cls.model.update(
                ...
                process_duation=cls.model.process_duation + duation,
            )
    ...
    def accessible4deletion(cls, doc_id, user_id):
        docs = (
            cls.model.select(cls.model.id)
            .join(Knowledgebase, on=(Knowledgebase.id == cls.model.kb_id))
            .where(cls.model.id == doc_id, Knowledgebase.created_by == user_id)
            .paginate(0, 1)
        )
        docs = docs.dicts()
        if not docs:
    ...
    def update_progress(cls):
        ...
        for d in docs:
            try:
                ...
                if finished and bad:
                    ...
                    if (
                        ...
                        queue_raptor_o_graphrag_tasks(
                            d, "graph_resolution", MSG["graph_resolution"]
                        )
                        prg = 0.98 * len(tsks) / (len(tsks) + 1)
                    elif (
                        d["parser_config"].get("graphrag", {}).get("use_graphrag")
                        ...
                        queue_raptor_o_graphrag_tasks(
                            d, "graph_community", MSG["graph_community"]
                        )
                        prg = 0.98 * len(tsks) / (len(tsks) + 1)
                    else:
                ...
                msg = "\n".join(sorted(msg))
                info = {
                    "process_duation": datetime.timestamp(datetime.now())
                ...
                cks.append(
                    {
                        ...
                        "title_tks": rag_tokenizer.tokenize(
                            re.sub(r"\.[a-zA-Z]+$", "", doc_nm[doc_id])
                        ),
                        "content_ltks": rag_tokenizer.tokenize(
                            "summary summarize 总结 概况 file 文件 概括"
                        ),
