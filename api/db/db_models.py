...
class User(DataBaseModel, UserMixin):
    ...
    color_schema = CharField(
        max_length=32, null=True, help_text="Bright|Dark", default="Bright", index=True
    )
    timezone = CharField(
        max_length=64,
        null=True,
    ...
    is_superuser = BooleanField(
        null=True, help_text="is root", default=False, index=True
    )
...
class Tenant(DataBaseModel):
    ...
    llm_id = CharField(
        max_length=128, null=False, help_text="default llm ID", index=True
    )
    embd_id = CharField(
        max_length=128, null=False, help_text="default embedding model ID", index=True
    )
    asr_id = CharField(
        max_length=128, null=False, help_text="default ASR model ID", index=True
    )
    ...
    rerank_id = CharField(
        max_length=128, null=False, help_text="default rerank model ID", index=True
    )
    tts_id = CharField(
        max_length=256, null=True, help_text="default tts model ID", index=True
    )
    parser_ids = CharField(
        max_length=256, null=False, help_text="document processors", index=True
    )
...
class TenantLLM(DataBaseModel):
    ...
    llm_factory = CharField(
        max_length=128, null=False, help_text="LLM factory name", index=True
    )
    model_type = CharField(
        max_length=128,
        null=True,
    ...
    llm_name = CharField(
        max_length=128, null=True, help_text="LLM name", default="", index=True
    )
...
class Knowledgebase(DataBaseModel):
    ...
    permission = CharField(
        max_length=16, null=False, help_text="me|team", default="me", index=True
    )
...
class Document(DataBaseModel):
    ...
    parser_id = CharField(
        max_length=32, null=False, help_text="default parser ID", index=True
    )
    parser_config = JSONField(null=False, default={"pages": [[1, 1000000]]})
    ...
    created_by = CharField(
        max_length=32, null=False, help_text="who created it", index=True
    )
    name = CharField(max_length=255, null=True, help_text="file name", index=True)
    location = CharField(
        max_length=255, null=True, help_text="where dose it store", index=True
    )
...
class File(DataBaseModel):
    id = CharField(max_length=32, primary_key=True)
    parent_id = CharField(
        max_length=32, null=False, help_text="parent folder id", index=True
    )
    ...
    name = CharField(
        max_length=255, null=False, help_text="file name or folder name", index=True
    )
    location = CharField(
        max_length=255, null=True, help_text="where dose it store", index=True
    )
...
class File2Document(DataBaseModel):
    ...
    document_id = CharField(
        max_length=32, null=True, help_text="document id", index=True
    )
...
class Dialog(DataBaseModel):
    ...
    tenant_id = CharField(max_length=32, null=False, index=True)
    name = CharField(
        max_length=255, null=True, help_text="dialog application name", index=True
    )
    ...
    rerank_id = CharField(
        max_length=128, null=False, help_text="default rerank model ID"
    )
...
class Conversation(DataBaseModel):
    ...
    name = CharField(
        max_length=255, null=True, help_text="converastion name", index=True
    )
...
class APIToken(DataBaseModel):
    ...
    dialog_id = CharField(max_length=32, null=False, index=True)
    source = CharField(
        max_length=16, null=True, help_text="none|agent|dialog", index=True
    )
...
class UserCanvas(DataBaseModel):
    ...
    canvas_type = CharField(
        max_length=32, null=True, help_text="Canvas type", index=True
    )
    dsl = JSONField(null=True, default={})
...
def migrate_db():
    with DB.transaction():
        ...
        try:
            migrate(
                migrator.alter_column_type(
                    ...
                    "api_key",
                    CharField(
                        max_length=1024, null=True, help_text="API KEY", index=True
                    ),
                )
            )
        except Exception:
        ...
        try:
            migrate(
                migrator.add_column(
                    "tenant_llm", "max_tokens", IntegerField(default=8192, index=True)
                )
            )
        except Exception:
            pass
        try:
            migrate(
                migrator.add_column(
                    "api_4_conversation", "dsl", JSONField(null=True, default={})
                )
            )
        except Exception:
            pass
        try:
            migrate(
                migrator.add_column(
                    "knowledgebase", "pagerank", IntegerField(default=0, index=False)
                )
            )
        except Exception:
            pass
        ...
        try:
            migrate(
                migrator.add_column(
                    "task",
                    "digest",
                    TextField(null=True, help_text="task digest", default=""),
                )
            )
        except Exception:
            pass
        ...
        try:
            migrate(
                migrator.add_column(
                    "task",
                    "chunk_ids",
                    LongTextField(null=True, help_text="chunk ids", default=""),
                )
            )
        except Exception:
            pass
        try:
            migrate(
                migrator.add_column(
                    ...
                    "user_id",
                    CharField(
                        max_length=255, null=True, help_text="user_id", index=True
                    ),
                )
            )
        except Exception:
        ...
        try:
            migrate(
                migrator.add_column(
                    "document", "meta_fields", JSONField(null=True, default={})
                )
            )
        except Exception:
            pass
