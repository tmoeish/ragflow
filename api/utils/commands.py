...
def reset_email(email, new_email, email_confirm):
    ...
    if str(new_email).strip() == str(email).strip():
        click.echo(
            click.style("Sorry, new email and old email are the same.", fg="red")
        )
        return
    user = UserService.query(email=email)
    if not user:
    ...
    if not re.match(r"^[\w\._-]+@([\w_-]+\.)+[\w-]{2,4}$", new_email):
        click.echo(
            click.style("sorry. {} is not a valid email. ".format(new_email), fg="red")
        )
        return
    ...
    if new_user:
        click.echo(
            click.style(
                "sorry. the account: [{}] is exist .".format(new_email), fg="red"
            )
        )
        return
