...
def create(tenant_id, chat_id):
    ...
    dia = DialogService.query(
        tenant_id=tenant_id, id=req["dialog_id"], status=StatusEnum.VALID.value
    )
    if not dia:
    ...
    conv = {
        ...
        "message": [
            {"role": "assistant", "content": dia[0].prompt_config.get("prologue")}
        ],
                ...
                if ele["type"] == "file":
                    if files is None or not files.get(ele["key"]):
                        return get_error_data_result(
                            f"`{ele['key']}` with type `{ele['type']}` is required"
                        )
...
def update(tenant_id, chat_id, session_id):
    ...
    if not DialogService.query(
        id=chat_id, tenant_id=tenant_id, status=StatusEnum.VALID.value
    ):
...
def chat_completion(tenant_id, chat_id):
    ...
    if not req.get("session_id"):
        req["question"] = ""
    if not DialogService.query(
        tenant_id=tenant_id, id=chat_id, status=StatusEnum.VALID.value
    ):
        return get_error_data_result(f"You don't own the chat {chat_id}")
    if req.get("session_id"):
        if not ConversationService.query(id=req["session_id"], dialog_id=chat_id):
            return get_error_data_result(
                f"You don't own the session {req['session_id']}"
            )
    if req.get("stream", True):
        resp = Response(
            rag_completion(tenant_id, chat_id, **req), mimetype="text/event-stream"
        )
...
def chat_completion_openai_like(tenant_id, chat_id):
    ...
    msg = [
        m
        for m in messages
        if m["role"] != "system" and (m["role"] != "assistant" or msg)
    ]
    ...
    if req.get("stream", True):
        ...
        resp = Response(
            streamed_response_generator(chat_id, dia, msg), mimetype="text/event-stream"
        )
        resp.headers.add_header("Cache-control", "no-cache")
        resp.headers.add_header("Connection", "keep-alive")
        resp.headers.add_header("X-Accel-Buffering", "no")
...
def agent_completions(tenant_id, agent_id):
    ...
    if req.get("stream", True):
        resp = Response(
            agent_completion(tenant_id, agent_id, **req), mimetype="text/event-stream"
        )
        resp.headers.add_header("Cache-control", "no-cache")
        resp.headers.add_header("Connection", "keep-alive")
        resp.headers.add_header("X-Accel-Buffering", "no")
...
def list_agent_session(tenant_id, agent_id):
    ...
    if request.args.get("desc") == "False" or request.args.get("desc") == "false":
        ...
        desc = True
    convs = API4ConversationService.get_list(
        agent_id, tenant_id, page_number, items_per_page, orderby, desc, id, user_id
    )
    if not convs:
        return get_result(data=[])
    for conv in convs:
...
def chatbot_completions(dialog_id):
    ...
    if req.get("stream", True):
        resp = Response(
            iframe_completion(dialog_id, **req), mimetype="text/event-stream"
        )
        resp.headers.add_header("Cache-control", "no-cache")
        resp.headers.add_header("Connection", "keep-alive")
        resp.headers.add_header("X-Accel-Buffering", "no")
...
def agent_bot_completions(agent_id):
    ...
    if req.get("stream", True):
        resp = Response(
            agent_completion(objs[0].tenant_id, agent_id, **req),
